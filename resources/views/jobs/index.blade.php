@extends('layout',['title'=>__('jobs.page_title')])
@section('content')
<div class="page-head">
...
<section class="job-summary" aria-label="{{ __('ui.ringkasan_peluang_kerja.6d3654ff') }}">
    ...
    <div><strong>{{ number_format($stats['locations']) }}</strong><span>{{ __('jobs.mukims_offered') }}</span></div>
</section>
...
<form class="panel job-filter-panel" method="get" action="{{ route('jobs.index') }}" role="search">
    ...
    <div class="form-group"><label for="job-sector">{{ __('jobs.job_category') }}</label><select class="select" id="job-sector" name="job_category"><option value="">{{ __('jobs.all_job_categories') }}</option>@foreach($jobCategories as $option)<option value="{{ $option }}" @selected(($filters['job_category']??'')===$option)>{{ __("jobs.categories.$option") }}</option>@endforeach</select></div>
    <div class="form-group"><label for="job-category">{{ __('ui.kategori_oku.5a4ba70d') }}</label><select class="select" id="job-category" name="category"><option value="">{{ __('ui.semua_kategori.3ee43aaf') }}</option>@foreach(['Semua','Fizikal','Pendengaran','Mental','Pembelajaran','Penglihatan'] as $option)<option value="{{ $option }}" @selected(($filters['category']??'')===$option)>{{ __("jobs.oku_categories.$option") }}</option>@endforeach</select></div>
    <div class="form-group"><label for="job-location">{{ __('jobs.mukim') }}</label><select class="select" id="job-location" name="location"><option value="">{{ __('jobs.all_besut_mukims') }}</option>@foreach($mukims as $mukim)<option value="{{ $mukim }}" @selected(($filters['location']??'')===$mukim)>{{ $mukim }}</option>@endforeach</select></div>
...
@forelse($jobs as $job)
@php $descriptionId='job-description-'.$job->id; $interest=$interestsByJob->get($job->id); @endphp
<article class="panel opportunity-card" aria-labelledby="job-title-{{ $job->id }}" aria-describedby="{{ $descriptionId }}">
    <div class="opportunity-top"><span class="company-initial" aria-hidden="true">{{ mb_substr($job->employer->company_name,0,1) }}</span><div><p>{{ $job->employer->company_name }}</p><h3 id="job-title-{{ $job->id }}">{{ $job->title }}</h3>@if($job->job_category)<small>{{ __("jobs.categories.{$job->job_category}") }}</small>@endif</div><span class="job-category">{{ __("jobs.oku_categories.{$job->oku_category_suitable}") }}</span></div>
    <div class="job-meta"><span>⌖ {{ $job->location }}</span><span>▣ {{ __("jobs.employment_types.{$job->employment_type}") }}</span><span>RM {{ number_format((float)$job->salary_min,0) }}{{ $job->salary_max?' – '.number_format((float)$job->salary_max,0):'+' }}</span></div>
    ...
    <div class="opportunity-footer">
        <span class="job-deadline {{ $job->application_deadline?->diffInDays(today(),false)>=-7?'is-soon':'' }}">
            @if($job->application_deadline)
                {{ __('jobs.deadline', ['date' => $job->application_deadline->format('d/m/Y')]) }}
            @else
                {{ __('jobs.applications_open') }}
            @endif
        </span>
        @if(auth()->user()->role==='oku_user')
            @if($canApply)
                @if($interest?->profile_shared_at)
                    <span class="badge">{{ __('jobs.profile_shared') }} · {{ __("jobs.statuses.{$interest->status}") }}</span>
                @else
                    <form class="job-interest-form" method="post" action="{{ route('jobs.interest',$job) }}">@csrf<label class="check-option"><input name="share_profile" type="checkbox" value="1" required><span>{{ __('jobs.share_consent') }}</span></label><button class="btn btn-primary" type="submit">{{ $interest?__('jobs.share_profile'):__('jobs.interested') }}</button></form>
                @endif
            @else
            <a class="btn btn-primary" href="{{ route('career-profile.show') }}">{{ __('ui.lengkapkan_profil.3dac4b06') }}</a>
            @endif
        @else
        <div class="job-management-actions"><span class="applications-count">{{ __('jobs.interest_count',['count'=>number_format($job->applications_count)]) }}</span>@if(auth()->user()->role!=='employer' || (int)auth()->user()->employer_id===(int)$job->employer_id)<a class="btn" href="{{ route('jobs.candidates.index',$job) }}">{{ __('jobs.view_candidates') }}</a><a class="btn" href="{{ route('jobs.edit',$job) }}">{{ __('ui.sunting.b7b0d4ed') }}</a>@endif</div>
        @endif
    </div>
</article>
@empty
