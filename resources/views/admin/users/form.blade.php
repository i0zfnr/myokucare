...
@endphp
<div class="page-head"><div><p class="eyebrow">Pentadbiran Sistem</p><h2>{{ $managedUser->exists?'Kemaskini Akaun':'Daftar Pengguna Baharu' }}</h2><p>Tetapkan identiti, peranan dan akses pengguna dengan berhati-hati.</p></div><a class="btn" href="{{ route('admin.users.index') }}">← Kembali</a></div>
...
<div class="form-group"><label for="managed-role">Peranan <span class="required-mark">*</span></label><select class="select" id="managed-role" name="role" data-role-select required>@foreach($roleLabels as $role=>$label)<option value="{{ $role }}" @selected($value('role','viewer')===$role)>{{ $label }}</option>@endforeach</select>@error('role')<span class="field-error">{{ $message }}</span>@enderror</div>
...
<div class="form-actions"><a class="btn" href="{{ route('admin.users.index') }}">Batal</a><button class="btn btn-primary" type="submit">{{ $managedUser->exists?'Simpan Perubahan':'Cipta Akaun' }}</button></div>
</form>
@endsection
