...
<div class="form-group"><label for="managed-role">Peranan <span class="required-mark">*</span></label><select class="select" id="managed-role" name="role" data-role-select required>@foreach($roleLabels as $role=>$label)<option value="{{ $role }}" @selected($value('role',request('role','oku_user'))===$role)>{{ $label }}</option>@endforeach</select>@error('role')<span class="field-error">{{ $message }}</span>@enderror</div>
...
<section class="panel form-section"><div class="form-section-head"><span>03</span><div><h3>Kebenaran Terperinci</h3><p>Pilih hanya tindakan yang diperlukan. Kebenaran data sensitif dan pemadaman kekal mesti diberi secara berasingan.</p></div></div>
